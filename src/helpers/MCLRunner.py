...
class MCLRunner(object):
    ...
    def _build_optimizer(self, model):
        ...
        optimizer = eval('torch.optim.{}'.format(self.optimizer_name))(
            model, lr=self.learning_rate, weight_decay=self.l2)
        return optimizer
    ...
    def fit(self, dataset: BaseModel.Dataset, epoch=-1) -> float:
        model = dataset.model
        if model.optimizer_1 is None or model.optimizer_2 is None:
            model.optimizer_1 = self._build_optimizer([{"params": model.i_embeddings.parameters()}, {"params": model.graph_encoder.parameters()},{"params": model.fusion_layer.parameters()},{"params": model.fusion_layer_re.parameters()},{"params": model.encoder.parameters()}])
            model.optimizer_2 = self._build_optimizer([{"params": model.aug_1.parameters()}, {"params": model.aug_2.parameters()}])
        ...
        for batch in tqdm(dl, leave=False, desc='Epoch {:<3}'.format(epoch), ncols=100, mininterval=1):
            ...
            for param in model.aug_1.parameters():
                param.requires_grad = False
            for param in model.aug_2.parameters():
                param.requires_grad = False
            out_dict = model(batch)
            loss = model.loss(out_dict, 'step1')
            loss.backward()
            model.optimizer_1.step()
            loss_append += loss.detach().cpu().data.numpy()
            ################################# step2 #################################
            ...
            for param in model.aug_2.parameters():
                param.requires_grad = True
            out_dict = model(batch)
            loss = model.loss(out_dict, 'step2')
            loss.backward()
            ...
            for param in model.aug_2.parameters():
                param.requires_grad = False
            out_dict = model(batch)
            loss = model.loss(out_dict, 'step3')
            loss.backward()
            model.optimizer_1.step()
            loss_append += loss.detach().cpu().data.numpy()
            loss_lst.append(loss_append/3)
    ...
    def predict(self, dataset: BaseModel.Dataset) -> np.ndarray:
        ...
        for batch in tqdm(dl, leave=False, ncols=100, mininterval=1, desc='Predict'):
            print(batch['history_items'].shape[1])
    ...
    def tune_red(self, dataset: BaseModel.Dataset):
        return self.evaluate(dataset, [5], ['NDCG'])
