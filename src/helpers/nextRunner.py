...
from models.nextK.bert4next import Discriminator


class nextRunner(object):
    @staticmethod
    ...
    def reward_get(self, d_model, ehrs, paths, labels):
        ehrs = torch.Tensor(ehrs).long().to(self.device)
        rewards_episode = [[] for i in range(len(ehrs))]
        ground_truths_hops = []
        for t in range(self.nextK):
            path = [i[:t + 1] for i in paths]
            label = labels[:, :t + 1].tolist()
            ground_truths = [[1] if p == h else [0] for p, h in zip(path, label)]
            ground_truths_hops.append(ground_truths)
            rewards = d_model.batchClassify(ehrs, path).detach().cpu().numpy().tolist()
            rewards_episode = [row + r for row, r in zip(rewards_episode, rewards)]
        return rewards_episode, ground_truths_hops

    def train(self, data_dict: Dict[str, BaseModel.Dataset]):
        model = data_dict['train'].model
        discriminator = data_dict['train'].discriminator
    ...
    def fit(self, dataset: BaseModel.Dataset, epoch=-1) -> float:
        ...
        for batch in tqdm(dl, leave=False, desc='Epoch {:<3}'.format(epoch), ncols=100, mininterval=1):
            ...
            rewards_episode, ground_truths_hops = model.get_reward(discriminator, out_dict)
            loss = model.get_loss(rewards_episode, ground_truths_hops, batch['nextK_items'])
            model.optimizer.zero_grad()
            loss.backward()
            model.optimizer.step()
            loss_lst.append(loss.detach().cpu().data.numpy())
            if num % 4 == 0:
                loss = discriminator.train_d_model(out_dict)
