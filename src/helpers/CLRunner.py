...
class CLRunner(object):
    ...
    def parse_runner_args(parser):
        ...
        parser.add_argument('--cl_num', type=int, default=20,
                            help='train time for collaborative learning')
        return parser
    ...
    def train_contractive(self, data_dict: Dict[str, BaseModel.Dataset]):
        model = data_dict['train'].model
        main_metric_results, dev_results = list(), list()
        self._check_time(start=True)
        try:
            for epoch in range(self.cl_num):
                # Fit
                self._check_time()
                gc.collect()
                torch.cuda.empty_cache()
                loss = self.fit_contractive(data_dict['train'], epoch=epoch + 1)
    ...
    def train_encoder(self, data_dict: Dict[str, BaseModel.Dataset]):
        model = data_dict['train'].model
        main_metric_results, dev_results = list(), list()
        self._check_time(start=True)
        try:
    ...
    def train(self, data_dict):
        self.train_contractive(data_dict)
        self.train_encoder(data_dict)
    ...
    def fit_contractive(self, dataset: BaseModel.Dataset, epoch=-1) -> float:
        model = dataset.model
        if model.optimizer is None:
            model.optimizer = self._build_optimizer(model)
        dataset.actions_before_epoch()  # must sample before multi thread start
        ...
        for batch in tqdm(dl, leave=False, desc='Epoch {:<3}'.format(epoch), ncols=100, mininterval=1):
            batch = utils.batch_to_gpu(batch, model.device)
            model.optimizer.zero_grad()
            loss = model.contractive_bed(batch)
            loss.backward()
            model.optimizer.step()
