...
class MMCrec(SequentialModel):
    ...
    def _define_params(self):
        ...
        self.fusion_re = Attention_fusion(self.emb_size)
        self.fusion = Attention_fusion(self.emb_size)
    ...
    def forward(self, feed_dict):
        ...
        graph_vectors = adj[history]
        graph_vector = self.g_encoder(graph_vectors, lengths)
        graph_emb = adj[i_ids]
        ...
        out_vector = self.fusion(graph_vector, i_vector)
        out_vector2 = self.fusion_re(i_vector, graph_vector)
        # out_vector2 = self.fusion_layer_re(torch.cat((graph_vector, i_vector), -1))
        # out_emb = self.fusion_layer(torch.cat((graph_emb, i_emb), -1))
        prediction = (out_vector[:, None, :] * graph_emb).sum(-1) + (out_vector2[:, None, :] * i_emb).sum(-1)
        out_dict = {'out_seq': out_vector, 'prediction': prediction}

        # augmentation
        if feed_dict['phase'] == 'train':
            logits = torch.matmul(out_vector2, self.i_embeddings.weight.transpose(0, 1))
            logits2 = torch.matmul(out_vector, adj.transpose(0, 1))

            loss = self.loss_fct(logits + logits2, i_ids[:, 0])

            out_dict['loss'] = loss
            aug1_history = feed_dict['history_items_a']  # bsz, history_max
            aug1_lengths = feed_dict['history_len_a']  # bsz
            aug2_history = feed_dict['history_items_b']  # bsz, history_max
            ...
            # graph
            g_aug1_his_vectors = adj[aug1_history]
            g_aug2_his_vectors = adj[aug2_history]
            aug1_his_vector = self.g_encoder(g_aug1_his_vectors, aug1_lengths)
            aug2_his_vector = self.g_encoder(g_aug2_his_vectors, aug2_lengths)
            out_dict['g_aug1_seq'] = aug1_his_vector
            out_dict['g_aug2_seq'] = aug2_his_vector
    ...
    def loss(self, out_dict: dict) -> torch.Tensor:
        ...
        g_seq_output1, g_seq_output2 = out_dict['g_aug1_seq'], out_dict['g_aug2_seq']
        nce_logits, nce_labels = self.info_nce(seq_output1, seq_output2, temp=self.tau, batch_size=seq_output1.shape[0],
                                               sim=self.sim)
        nce_logits2, nce_labels2 = self.info_nce(g_seq_output1, g_seq_output2, temp=self.tau,
                                                 batch_size=seq_output1.shape[0],
                                                 sim=self.sim)
        nce_logits3, nce_labels3 = self.info_nce(g_seq_output1, seq_output2, temp=self.tau,
                                                 batch_size=seq_output1.shape[0],
                                                 sim=self.sim)
        nce_loss1 = self.nce_fct(nce_logits, nce_labels)
        nce_loss2 = self.nce_fct(nce_logits2, nce_labels2)
        nce_loss3 = self.nce_fct(nce_logits3, nce_labels3)

        loss += self.lmd * (nce_loss1 + nce_loss2 + nce_loss3)
        # if mode == "step1":
...
class Attention_fusion(nn.Module):
    def __init__(self, emb_size):
        ...
        self.attention_linear = nn.Linear(emb_size, 1)

    def forward(self, h1, h2):
        # Compute attention scores for h1
        attention_scores = self.attention_linear(h1)  # shape: [batch_size, 1]
        attention_weights = F.softmax(attention_scores, dim=1)  # shape: [batch_size, 1]

        # Compute weighted sum of h1 and h2 based on attention_weights
        fused_output = attention_weights * h1 + (1 - attention_weights) * h2  # shape: [batch_size, h_dim]
